...
void BasicPipelineProgram::SetModelViewMatrix(const float * m) 
{
  ...
  GLuint programID = this->GetProgramHandle();
  h_modelViewMatrix = glGetUniformLocation(programID, "modelViewMatrix");
...
void BasicPipelineProgram::SetProjectionMatrix(const float * m) 
{
  ...
  // students need to implement this
  GLuint programID = this->GetProgramHandle();
  h_projectionMatrix = glGetUniformLocation(programID, "projectionMatrix");
